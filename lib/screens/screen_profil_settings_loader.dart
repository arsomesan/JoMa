...
class _ProfilSettingsLoaderState extends State<ProfilSettingsLoader> {
  ...
  Widget build(BuildContext context) {
    Profil user = data.profile[2];
    var name = user.vorname! + " " + user.name!;
    var stadt = user.adresse!.plz! +  " " + user.adresse!.ort!;
    var email = user.kontakt!.email;
    var tel = user.kontakt!.tel;
    var adresse = user.adresse!.strasse! + " " + user.adresse!.hausnummer!;
    var bild = user.bild;
    return Scaffold(
      appBar: AppBar(
                ...
                child: Container(
                  ...
                  child: TextButton(
                    ...
                    onPressed: () async {
                      ...
                      if (telCheck) {
                        await UserSimplePreferences.setTelefon(
                            TelController.text);
                      }

                      if (adressCheck) {
                        await UserSimplePreferences.setAdresse(
                            AdressController.text);
                      }

                      if (stadtCheck) {
                        await UserSimplePreferences.setStadt(
                            StadtController.text);
                      }
                    },
                    style: TextButton.styleFrom(
                      primary: Glovar.white,
